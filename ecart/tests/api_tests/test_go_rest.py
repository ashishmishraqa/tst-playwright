...
class TestApi:
    ...
    def test_user_creation(self, api_auth, get_schema, pytestconfig):
        ...
        self.log.info(
            f"POST request to {request_url} returned status code {http_response_code}"
        )
    ...
    def test_get_request(self, api_auth, pytestconfig):
        ...
        self.log.info(
            f"GET request to {request_url} returned status code {api_response.status_code}"
        )

        # 3. Assert: Check the response
        assert (
            api_response.status_code == 200
        ), "Error! response code is not matching as expected "
