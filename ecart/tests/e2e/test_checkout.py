...
from ecart.utilities.data_utils import fetch_products
...
class TestCheckout(BaseTest):
    ...
    def test_checkout(self, page, products, home_page):
        ...
        product_page = home_page.search_product(products["product"])
        # expect(product_page.page).to_have_title(f'Search - {product}')

        # 3. Validate if product is found after search
        product_page.expect_product_on_search_page(products["product"])
        ...
        assert (
            product_page.return_total_cart_amount() == products["expected_price"]
        ), f"Error! {products['product']} amount is not matching as expected"
